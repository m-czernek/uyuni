...
class ProductTest(unittest.TestCase):
    ...
    def test_parse_products(self):
        res4 = next(
            ...
            if p.friendly_name == "RES 4 i386" and p.arch == "i386"
        )
        self.assertEqual([], res4.extensions)
        self.assertEqual(Product.Status.AVAILABLE, res4.status)

        sles11sp3_s390x = next(
            p
            for p in self.products
            if p.friendly_name == "SUSE Linux Enterprise Server 11 SP3 s390x"
            and p.arch == "s390x"
        )
        ...
        sle_ha = next(
            p
            for p in sles11sp3_s390x.extensions
            if p.friendly_name
