...
def xccdf_eval(server_id, action_id, data={}):
    ...
    if len(profiles) != 1 or len(testresults) != 1:
        log_error(
            "Scap report containst multiple results", len(profiles), len(testresults)
        )
    _process_testresult(
        testresults[0], server_id, action_id, benchmark, profiles[0], data["errors"]
    )
...
def _create_rresult(testresult_id, result_label):
    ...
    h.execute(rr_id=rr_id, testresult_id=testresult_id, result_label=result_label)
    return rr_id
