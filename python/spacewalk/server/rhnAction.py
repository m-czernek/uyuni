...
def update_server_action(
    server_id, action_id, status, result_code=None, result_message=""
):
...
def _invalidate_action_recursive(server_id, action_id, h, h_child):
    ...
    if s_row and s_row["status"] != 3:
        ...
        update_server_action(
            ...
            status=3,
            result_code=-100,
            result_message="Prerequisite failed",
        )
...
def schedule_server_packages_update_by_arch(
    server_id,
    package_arch_ids,
    org_id=None,
    prerequisite=None,
    action_name="Package update",
):
    action_id = schedule_server_action(
        server_id,
