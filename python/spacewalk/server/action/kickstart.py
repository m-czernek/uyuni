...
def schedule_sync(server_id, action_id, dry_run=0):
    ...
    if not ks_package_profile:
        ...
        server_kickstart.schedule_config_deploy(
            server_id, action_id, kickstart_session_id, server_profile=server_profile
        )
        raise ShadowAction(
    ...
    installs, removes = server_packages.package_delta(
        server_profile, ks_package_profile
    )
    ...
    if not (installs or removes):
        ...
        server_kickstart.schedule_config_deploy(
            server_id, action_id, kickstart_session_id, server_profile=None
        )
        raise ShadowAction("Package sync not scheduled, nothing to do")

    log_debug(4, "Scheduling kickstart delta")
    server_kickstart.schedule_kickstart_delta(
        server_id, kickstart_session_id, installs, removes
    )
