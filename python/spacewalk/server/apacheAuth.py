...
def _verifyProxyAuthToken(auth_token):
    ...
    log_debug(
        5,
        #  pylint: disable-next=consider-using-f-string
        "proxy auth token: %s,  hostname: %s" % (repr(token), hostname or "n/a"),
    )
    ...
    if computed != signature:
        ...
        log_debug(
            4,
            #  pylint: disable-next=consider-using-f-string
            "Sent proxy signature %s does not match ours %s." % (signature, computed),
        )
        #  pylint: disable-next=consider-using-f-string
        headers["X-RHN-Proxy-Auth-Error"] = "%s:%s" % (
            1003,
...
def auth_client():
    ...
    token = dict(
        (k.lower(), v) for k, v in list(rhnFlags.get("AUTH_SESSION_TOKEN").items())
    )
    ...
    for k in (
        ...
        if k not in token:
            ...
            log_debug(
                4,
                "Declined auth of client for GET requests; " "incomplete header info.",
            )
            return 0
