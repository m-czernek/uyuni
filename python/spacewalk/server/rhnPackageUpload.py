...
from spacewalk.server.rhnLib import (
    get_package_path,
...
def relative_path_from_header(header, org_id, checksum_type=None, checksum=None):
    ...
    # header, use it
    if (
        header.packaging == "mpm"
        and "package_name" in list(header.keys())
        and header["package_name"]
    ):
        rel_path = relative_path_from_nevra_without_package_name(
            nevra, org_id, checksum_type, checksum
        )
        return os.path.join(rel_path, header["package_name"])

    return relative_path_from_nevra(
        nevra, org_id, header.packaging, checksum_type, checksum
    )


def relative_path_from_nevra(
    nevra, org_id, package_type=None, checksum_type=None, checksum=None
):
    # 4/18/05 wregglej. if 1051 is in the header's keys, then it's a nosrc package.
...
def relative_path_from_nevra_without_package_name(
    nevra, org_id, checksum_type, checksum
):
...
def push_package(a_pkg, org_id=None, force=None, channels=[], relative_path=None):
    ...
    if package.diff and not force and package.diff.level > 1:
        ...
        log_debug(
            1, "Packages too different", package.toDict(), "Level:", package.diff.level
        )
    ...
    if package.diff and not force and package.diff.level:
        ...
        if os.path.exists(orig_path) and path_dict["path"]:
            ...
            h_upd.execute(
                path=relative_path, ctype=a_pkg.checksum_type, csum=a_pkg.checksum
            )
    ...
    if not a_pkg.header.is_source:
        ...
        server_packages.processPackageKeyAssociations(
            a_pkg.header, a_pkg.checksum_type, a_pkg.checksum
        )
...
def save_uploaded_package(
    stream, nevra, org_id, packaging, checksum_type=None, checksum=None
):
    ...
    temp_stream = tempfile.NamedTemporaryFile(
        dir=temp_dir, prefix="-".join((nevra[0], nevra[2], nevra[3], nevra[4]))
    )
