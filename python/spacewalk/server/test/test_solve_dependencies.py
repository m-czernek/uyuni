...
class SolveDependenciesTestCase(unittest.TestCase):
    ...
    def testMakeEvr(self):
        ...
        assert (
            nlimit["epoch"] == "38"
            and nlimit["name"] == "mozilla"
            and nlimit["version"] == "1.5"
            and nlimit["release"] == "2.rhfc1.dag"
        )
    ...
    def testEvrFilterE(self):
        nlimitstr = "38:mozilla-1.5-2.rhfc1.dag"
        nlimit = rhnDependency.make_evr(nlimitstr)
        pack = self.solve_deps_with_limits(
            self.serv_id, [self.filename], 2, limit_operator="==", limit=nlimitstr
        )
        assert (
            pack[self.filename][0][1] == nlimit["version"]
            and pack[self.filename][0][2] == nlimit["release"]
            and pack[self.filename][0][3] == nlimit["epoch"]
        )
    ...
    def testEvrFilterGT(self):
        ...
        assert (
            rpm.labelCompare(
                (
                    pack[self.filename][0][3],
                    pack[self.filename][0][1],
