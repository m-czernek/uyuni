...
class ConfigManagement(configFilesHandler.ConfigFilesHandler):
    ...
    def management_list_channels(self, dict):
        ...
        return [
            x["label"]
            for x in rhnSQL.fetchall_dict(
                self._query_list_config_channels, org_id=self.org_id
            )
            or []
        ]
    ...
    def management_create_channel(self, dict):
        ...
        row = rhnSQL.fetchone_dict(
            self._query_lookup_config_channel,
            org_id=self.org_id,
    ...
    def management_remove_channel(self, dict):
        ...
        row = rhnSQL.fetchone_dict(
            self._query_config_channel_by_label,
            org_id=self.org_id,
    ...
    def management_list_file_revisions(self, dict):
        ...
        retval = [
            x["revision"]
            for x in rhnSQL.fetchall_dict(
                self._query_list_file_revisions,
                org_id=self.org_id,
                config_channel=config_channel,
                path=path,
            )
    ...
    def management_remove_file(self, dict):
        ...
        row = rhnSQL.fetchone_dict(
            self._query_lookup_config_file_by_channel,
            org_id=self.org_id,
            config_channel=config_channel,
            path=path,
        )
    ...
    def management_disable_file(self, dict):
        ...
        rhnSQL.execute(
            self._query_update_file_state,
            config_file_id=row["id"],
            state_id=state_id_dead,
        )
        rhnSQL.commit()
        return {}
    ...
    #  pylint: disable-next=dangerous-default-value,redefined-builtin
    ...
    def __attributes_differ(self, fsrc, fdst):
        ...
        return (
            (fsrc["filemode"] != fdst["filemode"])
            or (fsrc["label"] != fdst["label"])
            or (fsrc["username"] != fdst["username"])
            or (fsrc["groupname"] != fdst["groupname"])
            or (fsrc["selinux_ctx"] != fdst["selinux_ctx"])
        )

    def __header(self, path, fsrc, config_channel_src, fdst, config_channel_dst):
        """Returns diff like header for this two files."""
        template = (
            "--- %s\t%s\tattributes: %s %s %s %s\tconfig channel: %s\trevision: %s"
        )
        first_row = template % (
            path,
    ...
    def management_diff(self, dict):
        ...
        if fsrc["label"] == "symlink":
            if (fsrc["symlink"] != fdst["symlink"]) or self.__attributes_differ(
                fsrc, fdst
            ):
    ...
    def lookup_org_config_channel_by_name(self, config_channel):
        row = rhnSQL.fetchone_dict(
            self._query_org_config_channels,
            config_channel=config_channel,
            org_id=self.org_id,
        )
        if not row:
    ...
    def _check_user_role(self):
        ...
        raise rhnFault(4006, "User is not a allowed to manage config files")
