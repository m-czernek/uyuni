...
class BaseChecksumRowDumper(BaseRowDumper):
    def set_iterator(self):
        # checksums
        checksum_arr = [
            {"type": self._row["checksum_type"], "value": self._row["checksum"]}
        ]
...
class _ChannelDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        arr.append(
            SimpleDumper(
                self._writer, "rhn-channel-product-name", channel_product_details[0]
            )
        )
        arr.append(
            SimpleDumper(
                self._writer, "rhn-channel-product-version", channel_product_details[1]
            )
        )
        arr.append(
            SimpleDumper(
                self._writer, "rhn-channel-product-beta", channel_product_details[2]
            )
        )
        ...
        if comp_last_modified is not None:
            arr.append(
                SimpleDumper(
                    self._writer,
                    "rhn-channel-comps-last-modified",
        ...
        if modules_last_modified is not None:
            arr.append(
                SimpleDumper(
                    self._writer,
                    "rhn-channel-modules-last-modified",
        ...
        h.execute(channel_id=channel_id)
        arr.append(
            ChannelFamiliesDumper(self._writer, data_iterator=h, ignore_subelements=1)
        )
        ...
        if self.start_date:
            ...
            query_args.update(
                {"lower_limit": self.start_date, "upper_limit": self.end_date}
            )
        else:
    ...
    def _get_package_ids(self):
        if self.start_date and self.whole_errata:
            return self._get_ids(
                self._query_pkgids_by_date_whole_errata,
                self._query_get_pkgids_by_rhndate_whole_errata,
                self._query_get_package_ids,
            )
        else:
            return self._get_ids(
                self._query_get_package_ids_by_date_limits,
    ...
    def _get_errata_ids(self):
        return self._get_ids(
            self._query__get_errata_ids_by_limits,
    ...
    def _get_kickstartable_trees(self):
        ks_trees = self._get_ids(
            self._query_get_kickstartable_trees_by_limits,
...
class ChannelFamiliesDumper(BaseQueryDumper):
    ...
    def __init__(
        self, writer, data_iterator=None, ignore_subelements=0, null_max_members=1
    ):
...
class _PackageDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        arr.append(
            _ChecksumDumper(self._writer, data_iterator=ArrayIterator(checksum_arr))
        )
        ...
        mappings = [
            [
                "rhnPackageRequires",
                "rhn-package-requires",
                "rhn-package-requires-entry",
            ],
            [
                "rhnPackageProvides",
                "rhn-package-provides",
                "rhn-package-provides-entry",
            ],
            [
                "rhnPackageConflicts",
                "rhn-package-conflicts",
...
class _ChangelogEntryDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        arr.append(
            SimpleDumper(
                self._writer,
                "rhn-package-changelog-entry-time",
...
class _PackageFilesDumper(BaseDumper):
    ...
    def dump_subelement(self, data):
        ...
        d = EmptyDumper(self._writer, "rhn-package-file", attributes=data)
        d.dump()
...
class _ErratumDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        if sev:
            arr.append(SimpleDumper(self._writer, "rhn-erratum-severity", sev["label"]))
        else:
...
class _ErratumBugDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        arr = [
            ...
            SimpleDumper(
                self._writer, "rhn-erratum-bug-summary", self._row["summary"] or ""
            ),
...
class _ErratumFileEntryDumper(BaseChecksumRowDumper):
    ...
    def set_attributes(self):
        ...
        channels = " ".join([x["label"] for x in h.fetchall_dict() or []])
        if channels:
